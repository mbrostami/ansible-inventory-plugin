...
DOCUMENTATION = '''
    ...
    description:
        ...
        - Host entries can have multiple tags which will be used as group names. 
...
class InventoryModule(BaseFileInventoryPlugin):
                ...
                for key in group_data:
                    ...
                    if isinstance(group_data[key], NoneType):
                        ...
                        for host_pattern in group_data[key]:
                            ...
                            self._parse_tags(hosts, port, host_pattern, group_data[key])
    ...
    def _parse_tags(self, hosts, port, host_pattern, host_params):
        if host_params[host_pattern] is not None:
            if 'tags' in host_params[host_pattern]:
                for tag in host_params[host_pattern]['tags']:
                    if tag not in self.inventory.groups:
                        new_group = {
                            "hosts": {
                                host_pattern: host_params[host_pattern]
                            }
                        }
                        self._parse_group(tag, new_group)
                    else:
                        self._populate_host_vars(hosts, host_params or {}, tag, port)
